    ...
    class frontierdetection
    {
        public:
            frontierdetection(ros::NodeHandle private_nh)
            {
                ...
                planning_vxy_pub_ = private_nh.advertise<geometry_msgs::Twist>("/planning_vxy", 1);
        ...
        private:
            ...
            void obstacle_xy_callback(const nav_msgs::Path::ConstPtr& msg)
            {
                ...
                if(!get_obstacle_xy)
                {
                    ...
                    for(auto i:obstacle_xy_.poses)
                    {
                        ...
                        obstacle_id.push_back((int)((i.pose.position.x-x0_place)/image_resolution+global_costmap.info.width*((int)i.pose.position.y-y0_place)/image_resolution));
                    }
            ...
            void target_xy_callback(const geometry_msgs::Pose::ConstPtr& msg)
            {
                ...
                goal_index = (target_x_position-x0_place)/image_resolution+global_costmap.info.width*(target_y_position-y0_place)/image_resolution;
            }
            void place_xy_callback(const nav_msgs::Path::ConstPtr& msg)
            {
                if(get_place_xy)
                {
                    ...
                    {
                        playground_id=(place_x_position[i]-x0_place)/image_resolution+global_costmap.info.width*(place_y_position[i]-y0_place)/image_resolution;
            ...
            void CostmapSubCallback_global(const nav_msgs::OccupancyGrid::ConstPtr& map_msg)
            {
                ...
                for(int i = 0; i < voronoi_sizeX ; i++)
                {
                    for(int j = 0; j < voronoi_sizeY ; j++)
                    {
                        ...
                        {
                            global_costmap.data[i+j*voronoi_sizeX]=50;
                        }
                    }
                }
                for (int x=0; x<voronoi_sizeX; x++) 
            ...
            double target_x_position;//the x position of the target
            double target_y_position;//the y position of the target
            double image_resolution=0.5;
            double x0_place;// the min x position of the playground
            double y0_place;// the min y position of the playground
