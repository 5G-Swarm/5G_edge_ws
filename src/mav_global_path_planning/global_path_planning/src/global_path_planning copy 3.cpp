        ...
        private:
            ...
            void target_xy_callback(const geometry_msgs::Pose::ConstPtr& msg)
            {
                if(global_costmap.info.width!=0)
                {
                    ...
                    robot_index = (int)((target_x_position-x0_place)/image_resolution)+(int)global_costmap.info.width*int((target_y_position-y0_place)/image_resolution);//get the goal idx;
                }
            ...
            bool collision_check(nav_msgs::OccupancyGrid& global_costmap,int first_point, int second_point)
            {
                
                double x0,y0,x1,y1;
                x0 = first_point%(int)global_costmap.info.width;
                y0 = first_point/(int)global_costmap.info.width;
                x1 = second_point%(int)global_costmap.info.width;
                y1 = second_point/(int)global_costmap.info.width;
                // global_costmap.data[first_point]=100;
                // global_costmap.data[second_point]=100;
                // cout<<"data:"<<x0<<" "<<y0<<" "<<x1<<" "<<y1<<endl;
                if(x0==x1)
                {
                    double min_y = y0<y1? y0:y1;
                    double length= y0<y1? y1-y0: y0-y1;
                    for(int i=(int)min_y;i<(int)length;i++)
                    {
                        if(global_costmap.data[x0+i*global_costmap.info.width]==100)
                        {
                ...
                else
                {
                    ...
                    {
                        double idx,x_temp,y_temp;
                        x_temp = x0+(x1-x0)*(double)j/(double)step;
                        y_temp = y0+k*(x_temp-x0);
                        // cout<<"x_temp:"<<x_temp<<endl;
                        // cout<<"y_temp:"<<y_temp<<endl;
                        if(global_costmap.data[(int)x_temp+(int)y_temp*(int)global_costmap.info.width]==100)
                        {
                            cout<<"collision detected"<<endl;
                            return true;
                        }
                        // else
                        //     global_costmap.data[(int)x_temp+(int)y_temp*(int)global_costmap.info.width]==10;//test
                    }
                    return false;
                }
            }
            void plot_playground(nav_msgs::OccupancyGrid& global_costmap, std::vector<int> playground_ids)
            {
                ...
                for(int i=0;i<2;i++)//plot the 1-2 and 3-0
                {
                    ...
                    for(int j=0;j<step;j++)
                    {
                        ...
                        x_temp = x0+(x1-x0)*(double)j/(double)step;
                        y_temp = y0+k*(x_temp-x0);
                        global_costmap.data[(int)x_temp+(int)y_temp*(int)global_costmap.info.width]=100;
                    }
                }
                cout<<"plot 2"<<endl;
                ...
                for(int i=0;i<2;i++)//plot the 0-1 and 2-3
                {
                    ...
                    for(int j=0;j<step;j++)
                    {
                        ...
                        y_temp = y0+k*(x_temp-x0);
                        global_costmap.data[x_temp+y_temp*(int)global_costmap.info.width]=100;
                    }
                }
            }
            void CostmapSubCallback_global(const nav_msgs::OccupancyGrid::ConstPtr& map_msg)
            {
                ...
                if(get_place_xy&&get_obstacle_xy)
                {
                    ...
                    if(goal_index!=0)
                    {
                        ...
                        double distance = sqrt(pow(robot_index/(int)global_costmap.info.width-goal_index/(int)global_costmap.info.width,2)+pow(robot_index%(int)global_costmap.info.width-goal_index%(int)global_costmap.info.width,2));
                        ...
                        else if(distance<=distance_to_goal&&!collision_check(global_costmap, robot_index,goal_index))
                        {
                            cout<<"close to goal"<<endl;
                            geometry_msgs::Twist vel_msg;
                            double vx = (goal_index%(int)global_costmap.info.width-robot_index%(int)global_costmap.info.width)/distance;
                            double vy =(goal_index/(int)global_costmap.info.width-robot_index/(int)global_costmap.info.width)/distance;
                            if(vx<-1)
                                vx=-1;
                            else if(vx>1)
                                vx=1;
                ...
                if(robot_index!=0)//test
                {
                    for(int i=-2 ; i<3 ; i++)
                    {
                        for(int j=-2 ; j<3; j++)
                        {
                            if(i*global_costmap.info.width+robot_index>=0 && i*global_costmap.info.width+robot_index<global_costmap.data.size())
                            {
                                global_costmap.data[i*global_costmap.info.width+j+robot_index]=80;
                            }
                        }
                    }
                }
            ...
            void goal_callback(const geometry_msgs::PoseStamped::ConstPtr& goal_pose)
            {
                if(global_costmap.info.width!=0)
                {
                    ...
                    // if(global_costmap.info.width!=0)
                    //     if((a[1]/a[2])*global_costmap.info.width+a[0]/a[2]>0 &&(a[1]/a[2])*global_costmap.info.width+a[0]/a[2]<global_costmap.data.size())
            ...
            double distance_to_reach = 3;//the reach signal to the goal 
            double distance_to_goal = 20;//the close signal to the goal
            bool **map1=NULL;
            bool map1_generated=false;
