    ...
    class frontierdetection
    {
        ...
        private:
            ...
            void obstacle_xy_callback(const nav_msgs::Path::ConstPtr& msg)
            {
                ...
                if(!get_obstacle_xy&&global_costmap.info.width!=0)
                {
            ...
            void target_xy_callback(const geometry_msgs::Pose::ConstPtr& msg)
            {
                if(global_costmap.info.width!=0)
                {
                    ...
                    // goal_index = (int)((target_x_position-x0_place)/image_resolution)+(int)global_costmap.info.width*int((target_y_position-y0_place)/image_resolution);//todo:get the goal idx;
                }
