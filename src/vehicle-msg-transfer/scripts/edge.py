...
def parse_rtk(message, robot_id):
    print('get ----rtk', robot_id)
    relay_rtk(message, robot_id)
def relay_rtk(message, robot_id):
    global ifm_e2c_dict
    if robot_id in ifm_e2c_dict.keys():
        ifm_e2c_dict[robot_id].send_rtk(message)


def parse_pcd(message, robot_id):
    print('get -----------pcd', robot_id)
...
def parse_ldr(message, robot_id):
    print('get ----------------ldr', robot_id)
    relay_ldr(message, robot_id)
def relay_ldr(message, robot_id):
    global ifm_e2c_dict
    if robot_id in ifm_e2c_dict.keys():
        ifm_e2c_dict[robot_id].send_ldr(message)
...
class ServerR2E(Informer):
    ...
    def rtk_recv(self):
        self.recv('rtk', parse_rtk)

    def ldr_recv(self):
        self.recv('ldr', parse_ldr)
...
class ServerE2C(Informer):
    ...
    def send_rtk(self, message):
        self.send(message, 'rtk')

    def send_ldr(self, message):
        self.send(message, 'ldr')
