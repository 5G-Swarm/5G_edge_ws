...
class single_FormationFlyingEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=-self.max_accel, high=self.max_accel, shape=(1,self.n_agents*self.nu),dtype=np.float32)

        self.observation_space = spaces.Box(low=-np.Inf, high=np.Inf, shape=(1,self.n_agents*self.n_features),
                                            dtype=np.float32)
    ...
    def _get_obs(self):
        ...
        return self.feats.reshape(1,self.n_agents*self.n_features)
