...
#             cost_dist[i] = (self.dist_list[i]-self.pre_dist_list[i])*30
        
#         temp_dist_1 = np.linalg.norm(self.obs_dict[i,:2])
#         if temp_dist_1>self.radius/3:
#             cost_dist_agn2tar[i] = 0.3/(temp_dist_1+0.01) 
#         else: 
#             # import pdb;pdb.set_trace() 
...
#         cost_energy[i] = 0#np.linalg.norm(self.u[i]) 
...
#         reward_dict[i] = -(cost_dist[i] + cost_dist_agn2tar[i]+cost_distribution[i] + cost_energy[i])
...
#         for i in range(self.n_agents):
#             self.x[i,0] = np.random.uniform(-2,2)+team_center[0]
#             self.x[i,1] = np.random.uniform(-2,2)+team_center[1]
...
#         return np.hstack((self.obs_dict,self.pre_obs_dict))/self.radius#self.feats.reshape(1,self.n_agents*self.n_features)
...
#########################dec_ten_基本实现避障和均匀，还是仅观测最近的两个agent，难以向下兼容，但能向上兼容
...
#         #return (state_values, state_network)
#         return np.hstack((self.obs_dict,self.pre_obs_dict))#self.feats.reshape(1,self.n_agents*self.n_features)
...
#             for temp_dist_2 in list(self.adj_distance_matrix[i]):
#                 if temp_dist_2>self.radius:#self.radius*2#self.ag_threshold*2:#*1.5
#                     k = 0
#                 else:
#                     k = 3#2.5
#                 if temp_dist_2>0.5:
