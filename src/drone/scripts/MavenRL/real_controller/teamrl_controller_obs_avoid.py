...
start_episode = 0
obstacle_index_list = [8,9,10,13,14]
nonobstacle_index_list = [12,1,7,3,4]#12,13,3,4,5
...
def call_back_plan_vxy(ros_twist):
	real_env.target_center[3] = ros_twist.linear.x/2
	real_env.target_center[4] = ros_twist.linear.y/2
	print("plan_speed",ros_twist.linear.x,ros_twist.linear.y)
...
def call_back_target_groundtruth(msg_target_xy):
 
	# real_env.target_center[0] = msg_target_xy.position.x 
	# real_env.target_center[1] = msg_target_xy.position.y
...
def main(episodes):
	...
	target_xy_publisher = rospy.Publisher('/target_xy', Pose, queue_size=10)
	obstacle_xy_publisher = rospy.Publisher('/obstacle_xy', Path, queue_size=10)
	rospy.Subscriber("/plan_vxy", Twist, call_back_plan_vxy)
	...
	#####new added 
	rospy.Subscriber("/target_xy", Pose, call_back_target_groundtruth)
	...
	real_env.estimate_target_center[0] = 60
	real_env.estimate_target_center[1] = 0
	...
	while not rospy.is_shutdown():
		...
		real_env.target_center = real_env.target_center.copy()
		...
		if real_env.counter<3:
			temp_x_list = []
			temp_y_list = []
			for i in nonobstacle_index_list:
				temp_x_list.append(real_env.state_drones_dict[i][0]) 
				temp_y_list.append(real_env.state_drones_dict[i][1])
			real_env.target_center[0] = np.mean(temp_x_list)
			real_env.target_center[1] = np.mean(temp_y_list)
		...
		real_env.obstacle_index_list = obstacle_index_list
		real_env.render()
		#######publisher zone

		msg_target_xy = Pose()
		msg_target_xy.position.x = real_env.target_center[0]
		msg_target_xy.position.y = real_env.target_center[1]
		target_xy_publisher.publish(msg_target_xy)
		msg_obstacle_list = Path()
		for i in range(len(real_env.obstacle_index_list)):
			msg_pose = PoseStamped()
			# robot_id = real_env.id_list.index(i)
			# if i <3:
			# 	msg_pose.pose.position.x = real_env.obstacle_state[i,0]-15
			# 	msg_pose.pose.position.y = real_env.obstacle_state[i,1]-15
			# else:
			# 	msg_pose.pose.position.x = real_env.obstacle_state[i,0]+10
			# 	msg_pose.pose.position.y = real_env.obstacle_state[i,1]+10
			msg_pose.pose.position.x = real_env.obstacle_state[i,0]
			msg_pose.pose.position.y = real_env.obstacle_state[i,1]
			msg_obstacle_list.poses.append(msg_pose)
		obstacle_xy_publisher.publish(msg_obstacle_list)
