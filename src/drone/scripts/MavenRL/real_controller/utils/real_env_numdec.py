...
class Real_env(gym.Env): 
    ...
    def _get_obs_xy(self):
        ...
        for i in range(self.n_agents):
            ...
            robot_id = self.id_list[i]
            if robot_id in self.pre_id_list:
                pre_obs_list.append(list(self.pre_obs_dict[self.pre_id_list.index(robot_id)]))
            else:
                pre_obs_list.append(temp)


        self.obs_dict = np.array(obs_list) 
        self.pre_obs_dict = np.array(pre_obs_list)
        return_obs_xy = np.hstack((self.obs_dict,self.pre_obs_dict))
        self.pre_obs_dict = self.obs_dict.copy()
        self.pre_id_list = self.id_list
        self.pre_n_agents = self.n_agents 

        return return_obs_xy#

    def _preprocessAction(self,action_dict):
        action_dict = np.clip(action_dict,-self.ACCEL_BOUND,self.ACCEL_BOUND) 
        vel_output = self.feat_xy[:,2:4] + action_dict*0.1
    ...
    def render(self, mode='human'):
        ...
        if self.fig is None:
            ...
            # ax.add_patch(cir1)
            plt.ylim(-1.0 * self.r_max/2, 1.0 * self.r_max/2)
