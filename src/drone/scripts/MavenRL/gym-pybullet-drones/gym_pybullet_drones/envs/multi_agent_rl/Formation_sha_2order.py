...
class Formation_containment_2order(BaseMultiagentAviary):
    ...
    def __init__(self,
        ...
        self.target_center = np.array([np.random.uniform(-1,1),np.random.uniform(-1,1),0.0,0.0])#np.array([0,0,1])
        self.radius = 1
        self.counter = 0
        self.n_features = 4
        self.degree = 2

        self.state_drones = np.zeros((self.n_agents,4))
    ...
    def reset(self):
        ...
        self.counter = 0
        self.center_speed_x = float(np.random.uniform(-1,1,1))
        self.center_speed_y = float(np.random.uniform(-1,1,1))
...
##############################################
    ...
    def _computeObs(self):
        ...
        self.target_center = np.hstack((obs[:2],obs[10:12]))
        # self.target_center[:2] = -0.2*np.ones(2)#np.zeros(2) 

        for i in range(self.n_agents):
            # obs = self._clipAndNormalizeState(self._getDroneStateVector(i))
            obs = self._getDroneStateVector(i)
            self.state_drones[i, :] = np.hstack((obs[:2],obs[10:12]))#obs[:3]
        # import pdb;pdb.set_trace()
        adj_matrix = self.get_connectivity(self.state_drones[:,:2])
        for i in range(self.n_agents):
            ...
            dist0 = np.linalg.norm((self.state_drones[a[0],:2] - self.state_drones[i,:2]).squeeze())
            dist1 = np.linalg.norm((self.state_drones[a[1],:2] - self.state_drones[i,:2]).squeeze())
...
#############################################
    def _preprocessAction(self,
        ...
        tar_x_vel = self.center_speed_x*math.sin(self.counter/90)
        tar_y_vel = self.center_speed_y*math.cos(self.counter/90)
        # self.target_center[5] = 0.01
        
        tar_vel = np.array([tar_x_vel,tar_y_vel,0.1]) #target的速度也融入action指令中
        # import pdb;pdb.set_trace()
        print("action:",action)
        for k, a in action.items():
            ...
            rpm_k, _, _ = self.ctrl[int(k)].computeControl(control_timestep=self.AGGR_PHY_STEPS*self.TIMESTEP, 
                                                        ...
                                                        target_pos=np.array([state[0],state[1],self.INIT_XYZS[0,2]]), # same as the current position
                                                        target_rpy=np.array([0,0,state[9]]), # keep current yaw
                                                        target_vel=state[10:13]+0.2*a # target the desired velocity vector
                                                        ) 
            # import pdb;pdb.set_trace()
            ...
            state = self._getDroneStateVector(self.NUM_DRONES-1)
            rpm_k, _, _ = self.ctrl[self.NUM_DRONES-1].computeControl(control_timestep=self.AGGR_PHY_STEPS*self.TIMESTEP, 
                                                        cur_pos=state[0:3],
                                                        cur_quat=state[3:7],
                                                        cur_vel=state[10:13],
                                                        cur_ang_vel=state[13:16],
                                                        ...
                                                        target_rpy=np.array([0,0,state[9]]), # keep current yaw
                                                        target_vel=self.SPEED_LIMIT*tar_vel # target the desired velocity vector
                                                        ) 
            rpm[self.NUM_DRONES-1,:] = rpm_k
