...
class EvaluateMultiAgent(rllib.template.MethodSingleAgent):
    def __init__(self, config, writer):
        super(EvaluateMultiAgent, self).__init__(config, writer)
    ...
    def select_method(self):
        ...
        if method_name == 'PPO':
            ...
            from . import td3_dec
            self.critic = config.get('net_critic', td3_dec.Critic)(config).to(self.device)
            self.actor = config.get('net_actor', td3_dec.Actor)(config).to(self.device)
