...
model_name = "test"#"dec_ten_bullet"
restore = False
test_flag = True#False#True#False# 

model_num = -1#774600#971800#227900
steps_per_episode = 5000
load_path = "./results/test_ten/2o_ten_vel/saved_models"#"./results/single/2022-04-24-16:01:38----td3_gnn/saved_models"#"./results/tri/2022-04-24-17:49:06----td3_gnn/saved_models"
start_episode = 0
...
def main(episodes):
	...
	config.env = "formation_containment_2order-v0"#"formation_containment-v0"#"DecentralFormationFlying-v0"
	...
	for episode in range(start_episode+1,episodes):
		...
		for time in range(steps_per_episode):
			...
			temp = np.zeros((env.n_agents,1))#[[0.01],[0.01],[0.01],[0.01],[0.01]])
			action = np.concatenate((action_dict,temp),axis=1)*2
			# import pdb;pdb.set_trace()
			action = {i: action[i, :] for i in range(env.n_agents)}
				
			next_obs_dict, reward_dict, done_dict, _ = env.step(action)
