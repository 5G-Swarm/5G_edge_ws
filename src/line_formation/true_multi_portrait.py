...
def line_formation(self,real):
    ...
    if real==False:
        ...
        rospy.Subscriber("/iris_0/mavros/vision_pose/pose", PoseStamped, pose_callback0)
        rospy.Subscriber("/iris_1/mavros/vision_pose/pose", PoseStamped, pose_callback1)
        rospy.Subscriber("/iris_2/mavros/vision_pose/pose", PoseStamped, pose_callback2)
        drone_vel_pub0=rospy.Publisher('/iris_0/mavros/setpoint_velocity/cmd_vel_unstamped', Twist, queue_size=10)
    ...
    while not rospy.is_shutdown():
        if (time.time()-flight_time)<=wait_time:
            ...
            setpos1_z=pos1[2]+gz
            setpos2_z=pos2[2]+gz
            err1_0=np.array(pos1)-np.array(pos0)
            err2_0=np.array(pos2)-np.array(pos1)
        else:
            if (flag_z%2 or flag_x%2):
                ...
                v=[v0,v1,v2]
                if np.linalg.norm(np.array(v))<0.5:
                    ...
                    if np.linalg.norm(np.array(init_pos0)-np.array(pos0))>0.5 or np.linalg.norm(np.array(init_pos1)-np.array(pos1))>0.5:
                ...
                if np.linalg.norm(np.array(v))<0.5:
                    ...
                    print('flag',flag)
            formation=[[0,0],np.array(err1_0)-np.array(pos1)+np.array(pos0),np.array(err2_0)-np.array(pos2)+np.array(pos1)]
            for i in range(3):       
                drone_vel0 = TwistStamped()
                robot_id = NUM[i]
                drone_vel0.header = Header()
                drone_vel0.header.stamp = rospy.Time.now()
                drone_vel0.header.frame_id = str(robot_id)
                drone_vel0.twist.linear.x = kp_x *v[i][0]+np.clip(formation[i][0],-1,1)
                drone_vel0.twist.linear.y = kp_y *v[i][1]+np.clip(formation[i][1],-1,1)
